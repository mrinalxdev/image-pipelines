...
import (
	...
	"github.com/mrinalxdev/image-processing/processing"
)
...
func loadImage(paths []string) <-chan Job {
	...
	go func() {
		...
		for _, p := range paths {
			...
			job.Image = processing.ReadImage(p)
			out <- job
		}
		close(out)
	}()
...
func resize(input <-chan Job) <-chan Job {
	...
	go func() {
		...
		for job := range input { // Read from the channel
			job.Image = processing.Resize(job.Image)
			out <- job
		}
		close(out)
	}()
...
func convertToGrayscale(input <-chan Job) <-chan Job {
	...
	go func() {
		for job := range input { // Read from the channel
			job.Image = processing.Grayscale(job.Image)
			out <- job
		}
		close(out)
	}()
...
func saveImage(input <-chan Job) <-chan bool {
	...
	go func() {
		for job := range input { // Read from the channel
			processing.WriteImage(job.OutPath, job.Image)
			out <- true
		}
		close(out)
	}()
